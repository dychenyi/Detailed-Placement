...
public:
    ...
    cluster(cluster& rhs):width(rhs.width),e(rhs.e),q(rhs.q),x1(rhs.x1),myid(idnum),fixed(rhs.fixed),abandoned(false),prev(rhs.prev),next(rhs.next),celllist(rhs.celllist),y_pos(rhs.y_pos){
        if(idnum==10851)
            cout<<""<<endl;
        ++idnum;
    }
